...
[English](https://github.com/alibabacloud-automation/terraform-alicloud-end-to-end-tracing-diagnostics/blob/main/README.md) | 简体中文

本示例用于实现解决方案[端到端全链路追踪诊断](https://www.aliyun.com/solution/tech-solution/end-to-end-tracing-and-diagnostics)，这是一个全面的分布式系统监控解决方案，涵盖专有网络（VPC）、交换机（VSwitch）、云服务器（ECS）、关系型数据库（RDS）、缓存数据库（Redis）、消息队列（RocketMQ）、微服务引擎（MSE）等资源的创建与部署，旨在帮助用户快速搭建具备全链路追踪、性能监控和故障诊断能力的分布式应用架构。
